...
BASE=os.path.dirname(os.path.abspath(__file__)); DB=os.getenv('AME_DB_PATH',os.path.join(BASE,'ame.db')); BRIDGE_TOKEN=os.getenv('AME_BRIDGE_TOKEN',''); ADMIN_PIN=os.getenv('AME_ADMIN_PIN',''); app=Flask(__name__,static_folder='web',static_url_path='')
def now(): return datetime.now(timezone.utc).isoformat()
def db(): c=sqlite3.connect(DB); c.row_factory=sqlite3.Row; return c
def init():
 c=db(); c.executescript('''CREATE TABLE IF NOT EXISTS documents(id TEXT PRIMARY KEY,filename TEXT NOT NULL,size INTEGER DEFAULT 0,state TEXT NOT NULL DEFAULT 'waiting',driver TEXT,dt TEXT,message TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL); CREATE TABLE IF NOT EXISTS jobs(id TEXT PRIMARY KEY,document_id TEXT NOT NULL,action TEXT NOT NULL,state TEXT NOT NULL DEFAULT 'queued',payload TEXT,result TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL); CREATE TABLE IF NOT EXISTS bridge(id TEXT PRIMARY KEY,name TEXT,last_seen TEXT); CREATE TABLE IF NOT EXISTS accounts(id TEXT PRIMARY KEY,name TEXT NOT NULL,email TEXT,profile_id TEXT NOT NULL,active INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL); CREATE TABLE IF NOT EXISTS drivers(id TEXT PRIMARY KEY,name TEXT NOT NULL,account_id TEXT,active INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL); CREATE TABLE IF NOT EXISTS audit(id INTEGER PRIMARY KEY AUTOINCREMENT,event TEXT NOT NULL,detail TEXT,created_at TEXT NOT NULL);'''); c.commit(); c.close()
init()
def bridge_auth():
...
def submit(docid):
 ...
 c=db(); dr=c.execute('SELECT d.*,a.profile_id,a.email FROM drivers d LEFT JOIN accounts a ON a.id=d.account_id WHERE d.name=? AND d.active=1',(driver,)).fetchone(); row=c.execute('SELECT id FROM documents WHERE id=?',(docid,)).fetchone()
 if not row: c.close(); return jsonify(error='Document not found'),404
 if not dr: c.close(); return jsonify(error='Driver is not configured in Admin'),400
 jid=secrets.token_hex(12); t=now(); payload=json.dumps({'driver_id':dr['id'],'account_id':dr['account_id'],'profile_id':dr['profile_id'],'email':dr['email']})
...
def decision(docid):
 ...
 if not row: c.close(); return jsonify(error='Document not found'),404
 if choice=='discard': c.execute("UPDATE documents SET state='discarded',message='Existing ERP attachment kept; new scan discarded',updated_at=? WHERE id=?",(now(),docid)); c.commit(); c.close(); audit('discard',row['dt'] or ''); return jsonify(ok=True)
 jid=secrets.token_hex(12); t=now(); c.execute("UPDATE documents SET state='queued',message='Replace approved; waiting for office ERP worker',updated_at=? WHERE id=?",(t,docid)); c.execute('INSERT INTO jobs(id,document_id,action,state,payload,created_at,updated_at) VALUES(?,?,?,?,?,?,?)',(jid,docid,'replace','queued','',t,t)); c.commit(); c.close(); audit('replace',row['dt'] or ''); return jsonify(ok=True,job_id=jid)
@app.post('/api/bridge/register')
def bridge_register():
 bridge_auth(); d=request.get_json(silent=True) or {}; bid=str(d.get('id','office-pc'))[:80]; t=now(); c=db(); c.execute('INSERT INTO bridge(id,name,last_seen) VALUES(?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,last_seen=excluded.last_seen',(bid,str(d.get('name','AME Office PC'))[:120],t)); c.commit(); c.close(); return jsonify(ok=True)
...
def next_job():
 ...
 t=now(); c.execute("UPDATE jobs SET state='claimed',updated_at=? WHERE id=? AND state='queued'",(t,r['id'])); c.execute("UPDATE documents SET state='processing',message='Opening ERP and checking DT',updated_at=? WHERE id=?",(t,r['document_id'])); c.commit(); out=dict(r); c.close();
 try: out.update(json.loads(out.get('payload') or '{}'))
 except: pass
 return jsonify(job=out)
@app.post('/api/bridge/jobs/<jid>/result')
def job_result(jid):
 bridge_auth(); d=request.get_json(silent=True) or {}; t=now(); c=db(); r=c.execute('SELECT document_id FROM jobs WHERE id=?',(jid,)).fetchone()
 if not r:c.close(); return jsonify(error='Job not found'),404
 if d.get('existing'):
  msg='Existing signed DT found in ERP — choose Replace or Discard'; c.execute("UPDATE jobs SET state='waiting_decision',result=?,updated_at=? WHERE id=?",(msg,t,jid)); c.execute("UPDATE documents SET state='needs_decision',message=?,updated_at=? WHERE id=?",(msg,t,r['document_id']))
 else:
  ok=bool(d.get('ok')); state='done' if ok else 'error'; msg=str(d.get('message','ERP upload verified' if ok else 'ERP upload failed'))[:500]; c.execute('UPDATE jobs SET state=?,result=?,updated_at=? WHERE id=?',(state,msg,t,jid)); c.execute('UPDATE documents SET state=?,message=?,updated_at=? WHERE id=?',('uploaded' if ok else 'error',msg,t,r['document_id']))
